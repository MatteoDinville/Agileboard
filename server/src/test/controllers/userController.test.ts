...
describe('userController', () => {
	...
	it('getProfile - success', async () => {
		prismaMock.user.findUnique.mockResolvedValue({ id: 1, email: 'a@a.com', name: 'A', createdAt: new Date(), updatedAt: new Date() })
		await getProfile(req as AuthRequest, res as Response, next)
	...
	it('updateProfile - success', async () => {
		...
		prismaMock.user.findUnique.mockResolvedValue(null)
		prismaMock.user.update.mockResolvedValue({ id: 1, email: 'new@example.com', name: 'New', createdAt: new Date(), updatedAt: new Date() })
		await updateProfile(req as AuthRequest, res as Response, next)
		expect(res.json).toHaveBeenCalled()
	})
