...
describe('authController', () => {
	let req: Partial<Request> & { cookies?: any }
	...
	beforeEach(() => {
		...
		jwtMock.sign.mockReset?.(); jwtMock.verify.mockReset?.();
	...
	it('me - 401 when no userId', async () => {
		...
		anyReq.userId = undefined
		await authController.me(anyReq, res as Response)
		expect(res.status).toHaveBeenCalledWith(401)
	})
